...
filename = "./Projects/BinaryFiles/Someday.mp3"
...
with open(filename, 'rb') as mp3_file:
    ...
    if header[:5] == b'ID3\x03\x00':
        ...
        if header[5] & 0b01000000:
            ...
            mp3_file.seek(ext_size, SEEK_CUR) # seeking relative to current position of file pointer
        ...
        while True:
            ...
            if frame_id in frame_types:
                ...
                if frame_id.startswith(b'T'):  # a text field
                    ...
                    #print(mp3_file.read(1), 'big') # TODO: see what reading a line of binary does
                    ...
                    if mime_type.startswith('image/'):
                        ...
                        image_data = mp3_file.read(image_size) # read number bytes in the image from 2 lines above

                        # Create a file name from the picture name
                        image_type = mime_type.split('/')[-1] # could be empty ext type (would cause failure on windows)
                        
                        # Get filename part only (without the path)
                        base_filename = path.split(filename)[1] # 0 path; 1 filename

                        # Now remove the extension
                        base_filename = path.splitext(base_filename)[0] # 0 name; 1 extension (empty if none)

                        # Create normal string formating
                    ...
                    while next_byte and next_byte == b'\0': # empty bytes object is returned when .read() is called at end of file
                        next_byte = mp3_file.read(1)
                    ...
                    if next_byte != b'':
                        mp3_file.seek(-1, SEEK_CUR) # check previous byte
