...
def sort_key(d: dict) -> str:
    return d['country'] # used in sorted function to extract 'country' value


#columns = ['country', 'gold', 'silver', 'bronze', 'rank'] # for all fields
columns = ['country', 'gold', 'silver', 'bronze'] # for all fields

filename = 'country_medals.csv'
with open(filename, 'w', encoding='utf-8', newline='') as output_file:
    #writer = csv.DictWriter(output_file, fieldnames=columns, quoting=csv.QUOTE_NONNUMERIC) # use all proided fields
    writer = csv.DictWriter(output_file, fieldnames=columns, quoting=csv.QUOTE_NONNUMERIC, extrasaction='ignore') # don't include 'rank' by using ignore
    # default behavior when key is excluded is to raise error, using extrasaction='ignore' it will ignore this and let you print less fields than what are provided
    writer.writeheader()
    # write row by row manually
    #for row in medals_table:
    #    writer.writerow(row)
    # have writer iterate rows for you
    ##writer.writerows(medals_table) # must be list of dictionaries with same # fields as fieldnames
    writer.writerows(sorted(medals_table, key=sort_key)) # to a list of dictionaries, a key must be provided, this uses a function to extract country
